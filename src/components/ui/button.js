...
const Button = React.forwardRef((props, ref) => {
  const { asChild, ...restProps } = props; // destructuring props to get asChild and other props
  const Comp = asChild ? Slot : 'button';
  return (
    <Comp
      className={cn(buttonVariants(restProps))} // buttonVariants is called with props
      ref={ref}
      {...restProps} // spread other props to Comp
    />
  );
});
